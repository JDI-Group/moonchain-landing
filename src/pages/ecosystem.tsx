...
function Page() {
  ...
  return (
    <layouts.default>
      <div className="flex items-center border-b border-dashed border-white/20 mb-16">
        ...
        <Spline
          className="w-fit! h-fit! -mr-26"
          scene="https://prod.spline.design/KQ3MFPcGhZ7NFiN1/scene.splinecode"
        />
      </div>
      <div className="grid grid-cols-5 gap-8">
        {config.ecosystem.map(item => (
          <div key={item.title} className="col-span-2 flex flex-col" style={{ gridColumn: `span ${Math.min(item.items.length, 5)}` }}>
            <div className="text-2xl font-bold">{item.title}</div>
            <div className="text-lg text-white/50 mb-8">{item.description}</div>
            <div className="grid gap-8 flex-1" style={{ gridTemplateColumns: `repeat(${Math.min(item.items.length, 5)}, 1fr)` }}>
              {item.items.map(item => (
